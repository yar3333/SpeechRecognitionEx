...
export class WhisperExtrasSttProvider implements ISttProvider {
    ...
    loadSettings(settings: Record<string, any>) {
    ...
    async processAudio(audioblob: Blob) {
        ...
        if (!apiResult.ok) {
            window.toastr.error(apiResult.statusText, 'STT Generation Failed (Whisper Extras)', { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true });
